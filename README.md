...
* если при выполнении команды apt-get upgrade выдается ошибка Could not get lock /var/lib/dpkg/lock - open, то нужно найти подвисший процесс командой ps aux | grep -i apt и убить его kill -9 PID
4. Вернитесь в систему под пользователем ***teacher***
5. Запустите скрипт [wsite_install](https://raw.githubusercontent.com/koo2018/websitebuilding/master/wsite_install). Надо будет указать основной домен вашего сервера, пароль администратора MySQL, а также стандартный пароль, который получат пользователи по умолчанию.
6. Скрипт установит все пакеты, скачает WordPress, скачает и настроит скрипты автоматизации создния пользовательских аккаунтов. По завершению работы скрипт удалит сам себя и перезагрузит сервер. Вернитесь и начинайте работу с аккаунтами пользователей/студентов. 
